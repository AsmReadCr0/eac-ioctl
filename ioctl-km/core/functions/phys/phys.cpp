...
PVOID phys::split_memory(PVOID SearchBase, SIZE_T SearchSize, const void* Pattern, SIZE_T PatternSize)
{
...
NTSTATUS phys::get_mmpfndatabase()
{
...
uintptr_t phys::get_directory_table_base(void* process_base)
{
	if (!NT_SUCCESS(get_mmpfndatabase()))
		return 0;

	virt_addr_t virt_base{}; virt_base.value = process_base;
	...
	for (int i = 0;; i++) {
		...
		for (int j = 0; j < (elem->NumberOfBytes.QuadPart / 0x1000); j++, current_phys_address += 0x1000) {
			...
			if (pnfinfo->u4.PteFrame == (current_phys_address >> 12)) {
				MMPTE pml4e{};
				if (!NT_SUCCESS(rwx::read_phys_addr(PVOID(current_phys_address + 8 * virt_base.pml4_index), &pml4e, 8, &read)))
					continue;
				...
				MMPTE pdpte{};
				if (!NT_SUCCESS(rwx::read_phys_addr(PVOID((pml4e.u.Hard.PageFrameNumber << 12) + 8 * virt_base.pdpt_index), &pdpte, 8, &read)))
					continue;
				...
				MMPTE pde{};
				if (!NT_SUCCESS(rwx::read_phys_addr(PVOID((pdpte.u.Hard.PageFrameNumber << 12) + 8 * virt_base.pd_index), &pde, 8, &read)))
					continue;
				...
				MMPTE pte{};
				if (!NT_SUCCESS(rwx::read_phys_addr(PVOID((pde.u.Hard.PageFrameNumber << 12) + 8 * virt_base.pt_index), &pte, 8, &read)))
					continue;
